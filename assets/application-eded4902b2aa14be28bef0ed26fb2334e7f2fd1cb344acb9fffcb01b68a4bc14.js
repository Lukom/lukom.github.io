  ...
  publicMethod.prep = function (object) {
    ...
    callback = function () {
      ...
      complete = function () {
        ...
        $box.removeClass('colorbox-loading');
        $loadingOverlay.hide();
...
function doOnPageLoad(func) {
  $(document).ready(func);//.on('page:load', func);
}

doOnPageLoad(function() {
    $(".preview-images a, .preview-image").colorbox({current: "{current}/{total}"});

    hljs.configure({tabReplace: '    ',
                    useBR: true});
    $('.code').each(function(_, e) {hljs.highlightBlock(e)});
});
doOnPageLoad(function() {
  if ($('.avatar').length) {
    var addAvatarHint = function(avatars) {
      avatars.tipsy({
        gravity: 's', fade: true, html: true, delayIn: 500, delayOut: 500,
        title: function(){return 'Немаєш аватарки?<br/>Реєструйся на <a href="http://gravatar.com/" target="_blank">Граватарі</a>!';}
      });
    };
    addAvatarHint($('.avatar img'));
  }
});
